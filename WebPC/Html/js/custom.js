...
jQuery(window).load(function() {
   ...
   jQuery(".headerbar").load("headerbar.html");
	jQuery(".leftpanel").load("leftpanel.html");
	jQuery(".contentpanel").load("index.html");
...
jQuery(document).ready(function() {
   ...
   //导航局部加载页面
   jQuery('.nav-parent .children li,li.no-child').live('click', function() {
   		var th = jQuery(this);
   		var webUrl = th.attr("data-url");
		$("#O2OMenu").find("li").removeClass("active");
		var parent = th.parents("li.nav-parent");
		var iconClass = th.parents("li.nav-parent").find("i").attr("class");
		var text = th.parents("li.nav-parent").find("span").html();
		var RootText = "<li>"+text+"</li>";
		var subText = "<li>"+th.text()+"</li>";
		
		
		if(parent.length == 0){
			parent = th;
			iconClass = th.find("i").attr("class");
			text = th.find("span").html();
			var RootText = "<li>"+text+"</li>";
		}
		var CatalogText = "<span class='label'>你的位置:</span><ol class='breadcrumb O2ORoot'>"+RootText+subText+"</ol>";
        
		parent.addClass("active");
		th.addClass("active");
		$(".pageheader h2").html("<i class='"+iconClass+"'></i>"+th.text());
		$(".breadcrumb-wrapper").find(".label").html(CatalogText);
		jQuery(".contentpanel").load(webUrl);
   		
   });
   
   //指定DIV局部加载
   jQuery('.HrefFrame').live('click',function(){
   		var th = jQuery(this);
   		var webUrl = th.attr("data-url");
   		var loadDiv = th.attr("data-div");
   		var webText = th.attr("data-title");
   		
   		var webTitle = "<li>"+webText+"</li>";
   		if(webText!=""){
   			var iClass = jQuery(".pageheader h2 i").attr("class");
	   		jQuery(loadDiv).load(webUrl);
	   		jQuery(".O2ORoot").append(webTitle);
	   		$(".pageheader h2").html("<i class='"+iClass+"'></i>"+webText);
   		}else{
   			jQuery(loadDiv).load(webUrl);
   		}
   ...
   //弹出层
   jQuery(".winO2OPanel").live('click', function(){   		
   		var th = jQuery(this);
   		var webUrl = th.attr("data-url");
   		var webTitle = th.attr("data-title");
   		jQuery(".O2OWin-title").html(webTitle);
   		jQuery(".O2OWin").load(webUrl);
