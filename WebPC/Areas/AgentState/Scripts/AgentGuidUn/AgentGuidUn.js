...
var option = {
    callback: function (index, params) {
        ...
        $.getJSON("/AgentState/AgentGuidUn/GetList", allparams, function (data) {
            ...
            $.each(data, function (i, item) {
                ...
                if (item.Status == 2) {
                    ...
                    htmlArr.push("<td><a href='/AgentState/AgentGuidUn/Details?GuidId="+item.ID+"'>" + item.GuidName + "</a></td>");
...
$("#BatchClose").click(function () {
    if (ChectGuid() == "") {
        ...
        $.post("/AgentState/AgentGuidUn/BatchOperation", { ID: ChectGuid(), pd: 3 }, function (result) {
            if (result > 0) {
                $.message("操作成功！", true);
                loadDataCount();
            } else {
...
$("#tabbody").on("click", "[name=AgentEdit]", function () {
    ...
    $.windowBase({
        ...
        url: "/AgentState/AgentGuidUn/Update?GuidName=" + obj.GuidName + "&ID=" + obj.GuidID + "&TEl=" + obj.ConTEl + "&GuidType=" + obj.GuidType + "",
        width: 800,
        height: 400
    });
});
...
$("#tabbody").on("click", "[name=AgentPauseHf]", function () {
    if (confirm("你确定要恢复吗？")) {
        ...
        $.post("/AgentState/AgentGuidUn/Operation", { ID: AgentId, pd: 2 }, function (result) {
            if (result > 0) {
                $.message("操作成功！", true);
                loadDataCount();
            } else {
...
$("#tabbody").on("click", "[name=AgentPause]", function () {
    if (confirm("你确定要暂停吗？")) {
        var AgentId = $(this).attr("AgentId");
        $.post("/AgentState/AgentGuidUn/Operation", { ID: AgentId, pd: 1 }, function (result) {
            if (result > 0) {
                $.message("操作成功！", true);
                loadDataCount();
            } else {
...
$("#tabbody").on("click", "[name=AgentPauseDelete]", function () {
    if (confirm("你确定要关闭导购吗，关闭之后将不能恢复？")) {
        var AgentId = $(this).attr("AgentId");
        $.post("/AgentState/AgentGuidUn/Operation", { ID: AgentId, pd: 3 }, function (result) {
            if (result > 0) {
                $.message("操作成功！", true);
                loadDataCount();
            } else {
...
var loadDataCount = function () {
    ...
    $.getJSON("/AgentState/AgentGuidUn/GetCount", $("#conditionForm").serializeObject(), function (data) {
