var CommentsCreateSuccess = function (hxr) {
    if (hxr.Status == true) {
...
$('#BuyTime').datetimepicker({
    ...
    BeforeSelectData: function (a, b, c) {
        if (new Date(c) > new Date($('#CommentTime').val())) {
            $.message("购买时间必须小于评论时间", false);
            return false;
        }
        return true;
    },
...
function setSelectGoods(obj) {
    $.each(obj, function () {
        ...
        if (!o || o.length == 0) {
            ...
            //手动删除验证
            $("#GoodsBasicIDs").O2OtipsHide({ eleid: "GoodsBasicIDs" });
