           ...
           // alert("length: " + APGlobal.provJSON);
            if (APGlobal.provJSON === null) {
               ...
               // alert(" else " + APGlobal.provJSON);
                ...
                var provTemplate = strZX + strA_G + strH_K+ strL_S+ strT_Z+ strGAT;
                pane.html(provTemplate);
            }
        },
        fillTabPane: function(slevel) {
            var html = $.ajax({
                url:  addrJsonUrl+this.currentAddressId+".json",
                async: false
            }).responseText;
			...
			if(html){
				...
				if(data.cityList){
					...
					this.headerTmpl.find("ul").append(APGlobal.headTemplateCity);
					...
					this.headerTmpl.find(".county").parent().remove();
					this.headerTmpl.find(".town").parent().remove();
					this.headerTmpl.find("ul").append(APGlobal.headTemplateCounty);
					...
					this.headerTmpl.find(".town").parent().remove();
					this.headerTmpl.find("ul").append(APGlobal.headTemplateTown);
...
//				}
				...
				var headerTmplParentLen = this.headerTmpl.find("li").length;
				headerTmplParent = this.headerTmpl.find("li:eq("+(headerTmplParentLen-2)+")").find("a");
				headerTmplParent.html(parent.html());
...
//					}  else {
        ...
        moveNextLevel: function() {
            ...
            if ((this.addressLevel === this.pickerLevel)||(this.selectedValue.prov == "71" && isCantX)){
                ...
                this.hide();
                this.headerTmpl.find("li:last").find("a").html(curContent);
                this.hideLoading();
...
//								}
    ...
    var APGlobal = {
		...
		provId:null,
        provTemplate: "",
        headTemplate :  '<div class="top-tabs address-tabs"><ul class="tabs clearfix">'+
                            '<li  class="active"><a  class="prov" > 省份 </a><span class="icon-rArrow"> </span></li>'+
                          '</ul></div>',
		headTemplateCity:'<li> <a class="city" > 城市 </a><span class="icon-rArrow"> </span></li>',
        headTemplateCounty: '<li> <a class="county" > 区县 </a><span class="icon-rArrow"> </span></li>',
        headTemplateTown:  '<li> <a class="town" > 乡镇 </a><span class="icon-rArrow"> </span></li>',
    ...
    APGlobal.template = '<div class="address-picker " >'+
                              APGlobal.headTemplate +
                              APGlobal.contTemplate +
                         '</div>';

    function getHeadTemplate() {
        return  '<div class="top-tabs address-tabs"><ul class="tabs clearfix">'+
                     '<li  class="active"><a  class="prov" > 省份 </a><span class="icon-rArrow"> </span></li>'+
                    APTemplate.headTemplateTemp  +
                '</ul></div>';
    }
    function getTemplate() {
         return  '<div class="address-picker " >'+
                     getHeadTemplate() +
                     APGlobal.contTemplate +
                 '</div>';
    }
